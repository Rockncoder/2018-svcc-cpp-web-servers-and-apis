...
//    root@x0x0:~# curl localhost --request 'PUT' --request-target '/book/12'
  ...
  my_http_server.get("/1", [](auto & req, auto & session){
    cout << req << endl; // '/1'
    session.do_write(make_response(req, "<div><h1>Hello Code Campers</h1><div>\n"));
  });
  ...
  char* sPort = getenv("PORT");
  uint32_t iPort = static_cast<uint32_t>(sPort != nullptr? stoi(sPort): 8080);
  cout << "PORT = " << iPort << "\n";

  my_http_server.listen(address, iPort, [&my_http_server](auto & session){
